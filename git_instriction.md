...
## Cлияния веток 
Для того , чтобы слить ветки нужна команда git merge branch name.Вместо  branch name вводим название требуемой ветки.Эти действия требуется делать находясь на той ветке , куда будет сливаться ветка(добавляться информация)
![скрин3](t.png)
